 ...
 *
 * Creates the table set test suites expect. Extracted from globalSetup so it
 * can build the per-run template database that each Jest worker copies
 * (see tests/setup.js). All statements are idempotent.
 */

module.exports = async function createTestTables(Database) {
